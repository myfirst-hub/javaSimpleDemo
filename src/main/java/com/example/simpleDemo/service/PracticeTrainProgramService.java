...
public class PracticeTrainProgramService {
    ...
    public int importTestResultFromExcel(MultipartFile file, Long subjectId)
        ...
        try {
            ...
            Integer fullScore = utilCustom.extractFullScoreFromHeader(totalScoreHeaderCell);
            ...
            for (int i = 1; i <= sheet.getLastRowNum(); i++) { // 从第二行开始读取数据
                ...
                if (studentIdCell != null) {
                    ...
                    if (studentIdCell.getCellType() == CellType.FORMULA) {
                        ...
                        String cellValue = utilCustom.getCellValueAsString(studentIdCell);
                        if (cellValue != null && !cellValue.isEmpty()) {
                            try {
                                studentId = Long.parseLong(cellValue.trim());
                            } catch (NumberFormatException e) {
                ...
                if (testTimeCell != null) {
                    if (testTimeCell.getCellType() == CellType.NUMERIC) {
                        ...
                        String cellValue = utilCustom.getCellValueAsString(testTimeCell);
                        if (cellValue != null && !cellValue.isEmpty()) {
                            try {
                                // 支持 yyyy/MM/dd 格式
                ...
                if (testNameCell != null) {
                    testName = utilCustom.getCellValueAsString(testNameCell);
                }
                ...
                for (int j = 5; j < row.getLastCellNum() - 1; j++) { // 最后一列是总分，不包含
                    ...
                    if (headerRow != null) {
                        ...
                        if (headerCell != null) {
                            key = utilCustom.getCellValueAsString(headerCell);
                        }
                    }
                    ...
                    String value = "";
                    if (cell != null) {
                        value = utilCustom.getCellValueAsString(cell);
                    }
                    if (!first) {
                        attributesBuilder.append(",");
                    }
                ...
                if (totalScoreCell != null) {
                    if (totalScoreCell.getCellType() == CellType.NUMERIC) {
                        ...
                        String cellValue = utilCustom.getCellValueAsString(totalScoreCell);
                        if (cellValue != null && !cellValue.isEmpty()) {
